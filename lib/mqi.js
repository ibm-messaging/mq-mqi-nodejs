 ...
 *
 * @author Mark Taylor
 * @copyright Copyright (c) IBM Corporation 2017, 2024
 */
...
const mqotel = require("./mqiotel.js");
...
const GetAsync = require("./mqigeta.js");
...
function loadLibMulti() {
  ...
  const searchRedistFirst = process.env.MQIJS_PREFER_INSTALLED_LIBRARY == null;
...
function objInCB(jsObject) {
  if (jsObject != null && jsObject._mqQueueManager != null && jsObject._mqQueueManager._inCB) {
    return true;
  } else {
    return false;
  }
...
function connInCB(jsHConn) {
  if (jsHConn != null && jsHConn._inCB) {
    return true;
  } else {
    return false;
  }
...
exports.ConnxSync = function (jsqMgrName, jscno, cb) {
  ...
  const hConn = _connx(jsqMgrName, jscno, false, cb);
...
exports.Connx = function (jsqMgrName, jscno, cb) {
  ...
  const hConn = _connx(jsqMgrName, jscno, true, cb);
  log.traceExit("ConnxSync");
  if (!cb) {
    return hConn;
  }
...
exports.ConnSync = function (jsqMgrName, cb) {
  ...
  const hConn = _connx(jsqMgrName, null, false, cb);
...
exports.Conn = function (jsqMgrName, cb) {
  log.traceEntry("Conn");
  const hConn = _connx(jsqMgrName, null, true, cb);
  log.traceExit("Conn");
  if (!cb) {
    return hConn;
  }
...
function _connx(jsqMgrName, jscno, async, cb) {
  ...
  const savedOptions = jscno.Options;
  let options = mqt.flagsToNumber("MQCNO", jscno.Options);
  ...
  if (async) {
    mqnapi.Connx(jsqMgrName, jscno, function (result) {
      ...
      const err = new MQError(jsCc, jsRc, "CONNX");
      if (jsCc != MQC.MQCC_OK) {
        cb(err, null);
      } else {
        cb(null, hConn);
    ...
    const result = mqnapi.Connx(jsqMgrName, jscno);
...
exports.Disc = function (jsQueueManager, cb) {
  ...
  checkParamCBRequired(cb, true);
  let async = true;
  if (connInCB(jsQueueManager)) {
    async = false;
  }
  _disc(jsQueueManager, async, cb);
  log.traceExit("Disc");
};
...
function _disc(jsQueueManager, async, cb) {
  ...
  function discInternal() {
    ...
    let jsRc = MQC.MQRC_NONE;
    let jsCc = MQC.MQCC_OK;
    let savedHConn = null;
    let err;
    ...
    mqotel.dltOtelHandles(jsQueueManager);
    ...
    if (async) {
      ...
      const result = mqnapi.Disc(savedHConn);
...
exports.Open = function (jsQueueManager, jsod, jsOpenOptions, cb) {
  ...
  checkParamCBRequired(cb, true);
  let async = true;
  log.trace("Open",null,"Checking %o",jsQueueManager);
  if (connInCB(jsQueueManager)) {
    log.trace("Open",null,"Flipping async flag for %o",jsQueueManager);
    async = false;
  }
  open(jsQueueManager, jsod, jsOpenOptions, async, cb);
  log.traceExit("Open");
};
...
function open(jsQueueManager, jsod, jsOpenOptions, async, cb) {
  ...
  function openInternal() {
    ...
    if (async) {
      const openOptions =  mqt.flagsToNumber("MQOO", jsOpenOptions);
      mqnapi.Open(jsQueueManager._hConn, jsod, openOptions, function (result) {
        unlock(jsQueueManager._hConn);
        jsRc = result.jsRc;
        jsCc = result.jsCc;

        const jsObject = new MQObject(result.jsHObj, jsQueueManager, jsod.ObjectName);
        err = new MQError(jsCc, jsRc, "OPEN");

        if (jsCc != MQC.MQCC_OK) {
          cb(err, null);
        } else {
          mqotel.openOtel(jsQueueManager, jsObject, jsod, openOptions, null);
          cb(null, jsObject);
        }
      });
    } else {
      const openOptions = mqt.flagsToNumber("MQOO", jsOpenOptions);
      const result = mqnapi.Open(jsQueueManager._hConn,jsod,  openOptions);
      unlock(jsQueueManager._hConn);
...
exports.Close = function (jsObject, jsCloseOptions, cb) {
  ...
  close(jsObject, jsCloseOptions, async, cb);
  log.traceExit("Close");
};
...
function close(jsObject, jsCloseOptions, async, cb) {
  ...
  function closeInternal() {
    let err;
    let jsRc = MQC.MQCC_OK;
    let jsCc = MQC.MQRC_NONE;
    ...
    if (async) {
      ...
      const mqHObj = jsObject._hObj;
      const result = mqnapi.Close(jsObject._mqQueueManager._hConn, mqHObj, mqt.flagsToNumber("MQCO", jsCloseOptions));
      unlock(jsObject._mqQueueManager._hConn);
...
exports.Begin = function (jsQueueManager, cb) {
  ...
  const result = mqnapi.Begin(jsQueueManager._hConn);

  const jsRc = result.jsRc;
  const jsCc = result.jsCc;
  const err = new MQError(jsCc, jsRc, "BEGIN");
...
exports.Cmit = function (jsQueueManager, cb) {
  ...
  const result = mqnapi.Cmit(jsQueueManager._hConn);

  const jsRc = result.jsRc;
  const jsCc = result.jsCc;
  const err = new MQError(jsCc, jsRc, "CMIT");
...
exports.Back = function (jsQueueManager, cb) {
  ...
  const result = mqnapi.Back(jsQueueManager._hConn);

  const jsRc = result.jsRc;
  const jsCc = result.jsCc;
  const err = new MQError(jsCc, jsRc, "BACK");
...
exports.Sub = function (jsQueueManager, jsQueueObject, jssd, cb) { // -> fill in Hobj if managed, Hsub describing subscription
  ...
  sub(jsQueueManager, jsQueueObject, jssd, async, cb);
  log.traceExit("Sub");
};
...
function sub(jsQueueManager, jsQueueObject, jssd, async, cb) {
  ...
  let jsHObjQ = MQC.MQHO_UNUSABLE_HOBJ;
  if (jsQueueObject) {
    jsHObjQ = jsQueueObject._hObj;
  }
    ...
    if (async) {
      mqnapi.Sub(jsQueueManager._hConn,
        ...
        function (result) {
          ...
          const jsHObjSub = result.jsHObjSub;

          const err = new MQError(jsCc, jsRc, "SUB");

          const jsSubObject = new MQObject(jsHObjSub, jsQueueManager, jssd.ResObjectString);
          let jsPublicationQueueObject = jsQueueObject;
          ...
          if (jsCc != MQC.MQCC_OK) {
            ...
            if (jssd.Options & MQC.MQSO_MANAGED) {
              mqotel.openOtel(jsQueueManager, jsSubObject, null, 0, jsPublicationQueueObject);
            }
      ...
      const result = mqnapi.Sub(jsQueueManager._hConn,
        jssd,
        jsHObjQ);
      unlock(jsQueueManager._hConn);
...
exports.Stat = function (jsQueueManager, jsType, cb) {
  ...
  const jssts = new MQSTS.MQSTS();

  log.traceEntry("Stat");


  const result = mqnapi.Stat(jsQueueManager._hConn, jsType, jssts);

  const jsRc = result.jsRc;
  const jsCc = result.jsCc;

  const err = new MQError(jsCc, jsRc, "STAT");
...
exports.Put = function (jsObject, jsmd, jspmo, buf, cb) {
  ...
  let async = true;
  if (objInCB(jsObject)) {
    log.trace("Put",null,"Flipping async flag for %o",jsObject);
    async = false;
  }
  put(jsObject, jsmd, jspmo, buf, async, cb);
  log.traceExit("Put");
};
function put(jsObject, jsmd, jspmo, buf, async, cb) {
  ...
  function putInternal() {
    ...
    mqotel.putOtelTraceBefore(jsObject._mqQueueManager, jsmd, jspmo, buf);

    const mqmd = MQMD._newBuf();
    MQMD._copyMDtoBuf(jsmd, mqmd );

    const mqpmo = MQPMO._newBuf();
    ...
    if (async) {
      mqnapi.Put(jsObject._mqQueueManager._hConn,
        ...
        buf, function (result) {
          ...
          const jsRc = result.jsRc;
          const jsCc = result.jsCc;
          const err = new MQError(jsCc, jsRc, "PUT");

          MQMD._copyMDfromBuf(jsmd,mqmd);
          MQPMO._copyPMOfromBuf(jspmo,mqpmo);

          mqotel.putOtelTraceAfter(jsObject._mqQueueManager, jspmo) ;
          if (jsCc != MQC.MQCC_OK) {
            cb(err);
          } else {
            cb(null);
      ...
      const result = mqnapi.Put(jsObject._mqQueueManager._hConn,
        jsObject._hObj,
        mqmd,
        mqpmo,
        buf);
...
exports.Put1 = function (jsQueueManager, jsod, jsmd, jspmo, buf, cb) {
  ...
  let async = true;
  if (connInCB(jsQueueManager)) {
    // If this function is called inside a getCB function, we need to force it to run
    // synchronously. Otherwise there might be an MQRC_HCONN_ASYNC_ACTIVE failure. This
    // is going to be done for all the "async" MQI verbs.
    log.trace("Put1",null,"Flipping async flag for %o",jsQueueManager);
    async = false;
  }
  put1(jsQueueManager, jsod, jsmd, jspmo, buf, async, cb);
  log.traceExit("Put1");
};

function put1(jsQueueManager, jsod, jsmd, jspmo, buf, async, cb) {
  log.traceEntry("put1","async=%o",async);
  checkParam(jsQueueManager, MQQueueManager, "MQQueueManager");
  checkParam(jsmd, MQMD.MQMD, "MQMD");
  checkParam(jsod, MQOD.MQOD, "MQOD");
  checkParam(jspmo, MQPMO.MQPMO, "MQPMO");
  ...
  function put1Internal() {
    ...
    mqotel.putOtelTraceBefore(jsQueueManager, jsmd, jspmo, buf);
    const mqmd = MQMD._newBuf();
    MQMD._copyMDtoBuf(jsmd, mqmd );
    ...
    if (async) {
      mqnapi.Put1(jsQueueManager._hConn,
        ...
        function (result) {
          ...
          mqotel.putOtelTraceAfter(jsQueueManager, jspmo) ;

          const jsRc = result.jsRc;
          const jsCc = result.jsCc;
          const err = new MQError(jsCc, jsRc, "PUT1");
      ...
      const result = mqnapi.Put1(jsQueueManager._hConn,
        jsod,
        mqmd,
        mqpmo,
        buf);
...
exports.GetSync = function (jsObject, jsmd, jsgmo, buf, cb) {
  ...
  mqotel.getOtelSyncTraceBefore(jsObject._mqQueueManager, jsObject, jsgmo);
  ...
  const result = mqnapi.Get(jsObject._mqQueueManager._hConn,
    jsObject._hObj,
    mqmd,
    mqgmo,
    buf);
  ...
  const jsRc = result.jsRc;
  const jsCc = result.jsCc;
  let jsDatalen = result.jsDatalen;
  const err = new MQError(jsCc, jsRc, "GET");
  ...
  if (jsCc == MQC.MQCC_OK || jsRc == MQC.MQRC_TRUNCATED_MSG_ACCEPTED) {
    const removed = mqotel.getOtelTraceAfter(jsObject._mqQueueManager, jsObject, jsgmo, jsmd, buf, false, jsgmo.OtelOpts);
    log.debug("Bytes to remove: %d from %d %o",removed,jsDatalen,buf);
    if (removed > 0) {
      const n = buf.copy(buf,0,removed,jsDatalen);
      jsDatalen -= removed;
      log.debug("Bytes copied: %d new len %d %o",n,jsDatalen,buf);
    }
  }
...
exports.Get = function (jsObject, jsmd, jsgmo, cb) {
  ...
  mqotel.getOtelAsyncTraceBefore(jsObject._mqQueueManager, jsObject, jsgmo);
...
exports.Inq = function (jsObject, jsSelectors, cb) {
  ...
  if (jsCc == MQC.MQCC_OK) {
    const result = mqnapi.Inq(jsObject._mqQueueManager._hConn, jsObject._hObj, jsSelectors);
    jsRc = result.jsRc;
    jsCc = result.jsCc;
  }
  const err = new MQError(jsCc, jsRc, "INQ");
...
exports.Set = function (jsObject, jsSelectors, cb) {
  ...
  if (!badSelector) {
    const result = mqnapi.Set(jsObject._mqQueueManager._hConn, jsObject._hObj, jsSelectors, charAttrLen);
    jsRc = result.jsRc;
    jsCc = result.jsCc;
  } else {
    jsRc = MQC.MQRC_SELECTOR_ERROR;
  ...
  const err = new MQError(jsCc, jsRc, "SET");
...
exports.CrtMh = function (jsQueueManager, jscmho, cb) {
  ...
  const savedOptions = jscmho.Options;
  jscmho.Options = mqt.flagsToNumber("MQCMHO", jscmho.Options);

  const result = mqnapi.CrtMh(jsQueueManager._hConn, jscmho);

  jscmho.Options = savedOptions; // They are unchanged
  const jsRc = result.jsRc;
  const jsCc = result.jsCc;
  const jsHMsg = result.jsHMsg;

  const err = new MQError(jsCc, jsRc, "MQCRTMH");
...
exports.DltMh = function (jsQueueManager, jsHMsg, jsdmho, cb) {
  ...
  const savedOptions = jsdmho.Options;

  jsdmho.Options = mqt.flagsToNumber("MQDMHO", jsdmho.Options);

  const result = mqnapi.DltMh(jsQueueManager._hConn, jsHMsg, jsdmho);

  jsdmho.Options = savedOptions; // They are unchanged
  const jsRc = result.jsRc;
  const jsCc = result.jsCc;

  const err = new MQError(jsCc, jsRc, "MQDLTMH");
...
exports.SetMp = function (jsQueueManager, jsHMsg, jssmpo, jsName, jspd, value, cb) {
  ...
  const savedOptions = jssmpo.Options;
  jssmpo.Options = mqt.flagsToNumber("MQSMPO", jssmpo.Options);

  const savedPDOptions = jspd.Options;
  const savedPDCopyOptions = jspd.CopyOptions;

  jspd.Options = mqt.flagsToNumber("MQPD_OPTIONS", jspd.Options);
  jspd.CopyOptions = mqt.flagsToNumber("MQCOPY", jspd.CopyOptions);

  const result = mqnapi.SetMp(jsQueueManager._hConn, jsHMsg, jssmpo, jsName, jspd, value);
  ...
  const err = new MQError(jsCc, jsRc, "MQSETMP");
...
exports.InqMp = function (jsQueueManager, jsHMsg, jsimpo, jspd, jsName, valueBuffer, cb) {
  ...
  const savedOptions = jsimpo.Options;
  ...
  const result = mqnapi.InqMp(jsQueueManager._hConn, jsHMsg, jsimpo,
  ...
  const jsType = result.Type;
  const jsPropsLen = result.PropsLen;
  const returnedName = jsimpo.ReturnedName;
  if (jsCc == MQC.MQCC_OK) {

    const enc = jsimpo.ReturnedEncoding;
    const le = ((enc & MQC.MQENC_INTEGER_MASK) == MQC.MQENC_INTEGER_REVERSED);
    switch (jsType) {
  ...
  const err = new MQError(jsCc, jsRc, "MQINQMP");
...
exports.DltMp = function (jsQueueManager, jsHMsg, jsDmpo, jsName, cb) {
  ...
  const savedOptions = jsDmpo.options;
  jsDmpo.Options = mqt.flagsToNumber("MQDMPO", jsDmpo.Options);

  const result = mqnapi.DltMp(jsQueueManager._hConn, jsHMsg, jsDmpo, jsName);
  ...
  const err = new MQError(jsCc, jsRc, "MQDLTMP");
...
// Explicitly set pointers back into this module, to avoid circular dependencies
mqotel.mq.mqi = exports;
