 ...
 *
 * The package is based on exposing the full MQI. It uses essentially the
 ...
 * how to map to the native C libraries. This contrasts with the MQ Light API
 * that is also available for Node.js, but is tied to a publish/subscribe
 * messaging model.
 *
 * The verbs implemented here invoke
 ...
 * function.
 *
 * Unimplemented MQI verbs include MQSET, MQINQ, MQBEGIN, all of the
 * message property controls and MQSUBRQ.
 *
 * All structures are marked as sealed, so that application errors in spelling
 ...
 */
var libmqm = ffi.Library('libmqm_r', {
 ...
 *
 * Note: Unlike the Java equivalent, this does not accept regular expressions
 ...
 *
 * @param (String)
 *        jsqMgrName - the queue manager to connect to
 * @param (Object)
 *        jsCno - connection options
 * @param (callback)
 *        callback - optional. Invoked for errors and
 *        with a reference to the qmgr connection.
 * @return (MQQueueManager) - reference to the connection
 * @throws (MQError)
 ...
 *        jsQueueManager - reference to the queue manager
 * @param (callback)
 ...
 *
 * @param (MQQueueManager)
 *        jsQueueManager - reference to the queue manager (hConn)
 * @param (MQOD)
 ...
 * @return (MQObject) - reference to the opened object (hObj)
 * @throws (MQError)
 *         Container for MQRC and MQCC values
 * @throws (TypeError)
 *         When a parameter is of incorrect type
 ...
 *        jsQueueManager - reference to the queue manager (hConn)
 * @param (MQObject)
 *       jsQueueObject - the queue to which publications will be
 ...
 * @return (MQObject) - reference to the subscription (hSub)
 * @throws (MQError)
 *         Container for MQRC and MQCC values
 * @throws (TypeError)
 *         When a parameter is of incorrect type
 ...
 * @param (String or Buffer)
 *        buf - containing the message contents
 * @param (callback)
 *        callback - optional. Invoked for errors. No additional parameter
 *        on success.
 * @throws (MQError)
...
/**
 * GetDone -  Complete the Get processing and stop listening
 * for more messages.
 * This must not be called from within the callback message receiver.
 *
 * TODO: See if this can be solved (but it's to do with the threading
 * model) where the callback from the qmgr library is on a different
 * thread than the application execution thread.
 *
 * @param (MQObject)
 *        jsObject - reference to the opened object (hConn and hObj)
 * @param (callback)
 ...
 *
 * Calls the user-supplied callback function with the message
 * and control information.
 *
 * TODO: What about event callbacks, not just messages
 */
function mqiCB(hConn,pmqmd,pmqgmo,pbuf,pmqcbc) {
  ...
  var mqmd = ref.deref(pmqmd);
  var mqgmo = ref.deref(pmqgmo);
  ...
  if (c) {
    ...
    try {
      if (mqcbc.CompCode != MQC.MQCC_OK) {
        ...
        c.cb(null,c.object,jsgmo,jsmd,buf.slice(0,len));
      }
      c.inCB = false;
    } catch (err) {
      c.inCB = false;
...
function callbackRegistration(op,jsObject,jscbd,jsmd,jsgmo) {
  ...
  if (op == MQC.MQOP_REGISTER) {
    ...
    mqCbd.CallbackArea = Buffer.from(JSON.stringify(jsObject));
