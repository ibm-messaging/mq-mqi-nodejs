...
exports.MQAttr = function (selector,value) {
  /** The MQIA/MQCA selector value. For example MQIA_INIBIT_PUT
  @member {number}
  */
...
exports.Get = function(jsObject,jsmd,jsgmo,cb) {
  ...
  var c = setUserContext(jsObject,cb); // Save information needed for the callback

  try {
    callbackRegistration(c,MQC.MQOP_REGISTER,jsObject,jscbd,jsmd,jsgmo);
...
exports.GetDone = function(jsObject, cb) {
  ...
  if (!userContext) {
    ...
    throw err;
  } else if (userContext && userContext.inCB) {
    err = new MQError(MQC.MQCC_FAILED,MQC.MQRC_HCONN_ASYNC_ACTIVE,"GetDone");
    throw err;
  }
  ...
  try {
    if (jsObject) {
      ...
      callbackRegistration(null,MQC.MQOP_DEREGISTER,jsObject,jscbd);
    }
  } catch (err) {
    if (cb) {
       cb(err);
...
function mqiCB(hConn,pmqmd,pmqgmo,pbuf,pmqcbc) {
  ...
  try {
    if (p) {
      s = ref.readCString(p);
      jsObject = JSON.parse(s);
    } else {
      console.log("Callback Area is null");
    }
  } catch (err) {
     console.log("Cannot parse callbackArea %j",s);
  }
 ...
 */
function callbackRegistration(userContext, op,jsObject,jscbd,jsmd,jsgmo) {
  checkParam(jsObject,MQObject, 'MQObject', false);
  ...
  if (op == MQC.MQOP_REGISTER) {
    ...
    var b = ref.allocCString(JSON.stringify(jsObject));
    mqCbd.CallbackArea = b;
    mqCbd.CallbackFunction = fficb;
    // Keep a reference to the buffer to try to avoid GC
    if (userContext) {
      userContext.cbPtr = b;
    }
  } else {
