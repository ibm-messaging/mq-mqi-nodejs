 ...
 * This variable is exported so someone can override it if necessary.
 */
const getLoopIntervalMsMin = 250;
const getLoopIntervalMsDefault = 10 * 1000; // Default to 10 seconds
// This is in exports because it's the original implementation without the
// setter function. Making it private variable would be a breaking API change.
exports.getLoopIntervalMs = getLoopIntervalMsDefault;

// Whereas this parameter has never been exposed directly.
const maxConsecutiveGetsDefault = 100;
var maxConsecutiveGets = maxConsecutiveGetsDefault;
...
exports.setPollTime = function(v) {
  ...
  if (!v) {
    v = getLoopIntervalMsDefault;
  }
  exports.getLoopIntervalMs = v;
};
...
exports.Get = function(jsObject,jsmd,jsgmo,cb) {
  ...
  if (getLoop == null) {
    if (exports.getLoopIntervalMs < getLoopIntervalMsMin) {
      exports.getLoopIntervalMs = getLoopIntervalMsMin;
    }
    lastIntervalMs = 0;
  ...
  if (contextMap.size > 0) {
    ...
    for (var c of values) {
      ...
      do {
        ...
        if (consecutiveGets >= maxConsecutiveGets) {
          hitMaxGets = true;
        }
      } while (messageRetrieved && consecutiveGets < maxConsecutiveGets);
    }
    if (hitMaxGets) {
      // If any of the hObjs in the set has had a max retrieval count, then
      // go through the full set again immediately
      lastIntervalMs = 0;
      getLoop = setTimeout(getLoopFunction,0);
    } else {
      lastIntervalMs = exports.getLoopIntervalMs;
      getLoop = setTimeout(getLoopFunction,exports.getLoopIntervalMs);
    }
  } else {
    getLoop = null;
  }
