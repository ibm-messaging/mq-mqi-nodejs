 ...
 * Changes to it are only effective the next time the list of
 * GET requests changes from zero to non-zero (the setInterval is
 * restarted).
 */
exports.getLoopIntervalMs = 1000;
...
exports.Get = function(jsObject,jsmd,jsgmo,cb) {
  ...
  if (getLoop == null) {
    ...
    getLoop = setInterval(getLoopFunction,exports.getLoopIntervalMs);
  }
};
...
function getLoopFunction() {
  ...
  var lastIntervalMs = exports.getLoopIntervalMs;

  if (contextMap.size > 0) {
    // Do this for all active hObj requests
    for (var c of contextMap.values()) {
      do {
        messageRetrieved = false;
        if (c) {
          // Create copies of the original request parameters as they
          // may be modified during the GET processing
          ...
          if (c.WaitInterval >=0) {
            ...
            if (c.WaitInterval <=0 && !messageRetrieved) {
    ...
    // If there are no waiting GETters, then cancel this timer.
