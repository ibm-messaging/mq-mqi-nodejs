 ...
 */
const padding16 = " ".repeat(16);
const padding64 = padding16 + padding16 + padding16 + padding16 +  padding16;
const padding256 = padding64 + padding64 + padding64 + padding64;
const padding1024 = padding256 + padding256 + padding256 + padding256;
...
/* For backwards compatibility, this can be set to 'true' in which case
...
function loadLib(dir,name) {
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      ...
      var ldPath=process.env['DYLD_LIBRARY_PATH'];
      log.debug("MQ library '%s' loaded from %s combined with [%s]",name,
                                                                (dir==null)?"<<default path>>":dir,
                                                                (ldPath==null)?"<<empty DYLD path>>":ldPath);
    } else {
      log.debug("MQ library '%s' loaded from %s",name,(dir==null)?"<<default path>>":dir);
    }

    log.traceExit('loadLib');
    return null;
  } catch(err) {
    log.debug("MQ library '%s' failed to be loaded from %s. Error %s",name,(dir==null)?"<<default path>>":dir,err.message);
    log.traceExitErr('loadLib',err);
    return err;
  }
}
...
// usual check
function checkParamArray(b,t,m,o) {
  if (!Array.isArray(b)) {
    checkParam(b,t,m,o);
  }
}
...
//  log.debug("Lock tried for hConn: " + hc + " rc: " + rc);
  return rc;
}

function lock(hc) {
//  log.debug("Lock taken for hConn: " + hc);
  hConnsInUse.set(hc, Date.now());
}

function unlock(hc){
//  log.debug("Lock freed for hConn: " + hc);
...
exports.ConnxSync = function (jsqMgrName, jsCno, cb) {
  ...
  checkParamCB(cb);
  var hConn = connx(jsqMgrName,jsCno,false,cb);
...
exports.Connx = function (jsqMgrName, jsCno, cb) {
  ...
  var hConn = connx(jsqMgrName,jsCno,!tuningParameters.syncMQICompat,cb);
  log.traceExit('Connx');
  if (!cb) {
    return hConn;
  }
...
exports.Conn = function (jsqMgrName, cb) {
  log.traceEntry('Conn');
  var hConn = exports.Connx(jsqMgrName,null,cb);
  log.traceExit('Conn');
  if (!cb) {
    return hConn;
  }  
...
exports.ConnSync = function (jsqMgrName, cb) {
  ...
  var hConn = exports.ConnxSync(jsqMgrName,null,cb);
...
function open(jsQueueManager,jsod,jsOpenOptions,useAsync, cb) {
  ...
  function openInternal () {
    ...
    if (useAsync) {
      libmqm.MQOPEN.async(jsQueueManager._hConn,mqod.ref(),u.flagsToNumber('MQOO',jsOpenOptions),mqHObj,mqCc,mqRc, function(error,resp) {
        unlock(jsQueueManager._hConn);
      ...
      libmqm.MQOPEN(jsQueueManager._hConn,mqod.ref(),u.flagsToNumber('MQOO',jsOpenOptions),mqHObj,mqCc,mqRc);
      unlock(jsQueueManager._hConn);
...
function close(jsObject,jsCloseOptions,useAsync,cb) {
  ...
  function closeInternal () {
    ...
    if (useAsync) {
      libmqm.MQCLOSE.async(jsObject._mqQueueManager._hConn,mqHObj,u.flagsToNumber('MQCO',jsCloseOptions), mqCc,mqRc, function(error,resp) {
      ...
      libmqm.MQCLOSE(jsObject._mqQueueManager._hConn,mqHObj,u.flagsToNumber('MQCO',jsCloseOptions), mqCc,mqRc);
      unlock(jsObject._mqQueueManager._hConn);
    ...
    for (i=0;i<jsSelectors.length;i++) {
      ...
      if (selector >= MQC.MQIA_FIRST && selector <= MQC.MQIA_LAST) {
        ...
        if (charLen > 0) {
          ...
          charIndex += charLen;
	  if (selector == MQC.MQCA_INITIAL_KEY) {
...
exports.SetMp = function(jsQueueManager,jsHMsg,jssmpo,name,jspd,value,cb) {
  ...
  if (typeof value == 'string') {
    ...
    err = new MQError(MQC.MQCC_FAILED,MQC.MQRC_PROPERTY_TYPE_ERROR,"MQSETMP");
