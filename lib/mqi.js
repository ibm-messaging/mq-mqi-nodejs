...
function loadLib(dir,name) {
  ...
  try {
    ...
    debugLog("MQ library loaded from %s %s",dir,name);
    return null;
  } catch(err) {
    return err;
  }
...
var messagesRetrievedInHObjPoll = 0;
...
function pollPerHconn(hConn) {
  ...
  if (hObjsToPoll.length === 0) {
    hConnsInPoll.delete(hConn);
    debugLog('The hConn has no associate hObj outstanding');
    return;
  }
...
function pollHobjs(hConn) {
  let hObjsToPoll = getAllContextsForPoll(hConn);
  let nonWaitingContexts = getRunningContexts(hObjsToPoll);
  messagesRetrievedInHObjPoll = 0;
  ...
  let nonWaitingContextsLen = nonWaitingContexts.length;
  // And add further randomness that seemed to be helpful
  let randomIndex = Math.floor(Math.random() * nonWaitingContextsLen);
  debugLog(util.format("pollHObjs: About to work on #%d from %d for %d",randomIndex,nonWaitingContextsLen,hConn));
  initGet(nonWaitingContexts[randomIndex]);
}
 ...
 function pollHObj(c,jsmd,jsgmo) {
   ...
   function pollHObjInternal() {
     ...
     debugLog(util.format("PollHObjInternal: q = %s Get/HConn %d Get/HObj %d",
         c.object._name,messagesRetrievedInHConnPoll,messagesRetrievedInHObjPoll));
     ...
     libmqm.MQGET.async(c.object._mqQueueManager._hConn,
       ...
       if (c.cb) {
         if (jsCc != MQC.MQCC_OK) {
           ...
           messagesRetrievedInHObjPoll += 1;
           messagesRetrievedInHConnPoll += 1;

           debugLog(util.format("Retrieved %d for queue %s",messagesRetrievedInHObjPoll,c.object._name));
           if (messagesRetrievedInHObjPoll < tuningParameters.maxConsecutiveGets) {
             return setTimeout(pollHObjInternal,0);
           }
 ...
 * or with the IBMMQ_DEBUG environment variable.
 */
var debugEnabled = null;
function debugLog(fmt,...args) {
  if (debugEnabled == null) {
    if (tuningParameters.debugLog || process.env['IBMMQ_DEBUG'] != null) {
      debugEnabled = true;
    } else {
      debugEnabled = false;
    }
