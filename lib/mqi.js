...
function loadLibMulti() {
  ...
  if (process.platform === 'win32') {
    ...
    // Try standard installation directories for MQ
    if (err) {
      err = mqnapi.LoadLib("/opt/mqm/lib64", l);
    }
    if (err) {
      err = mqnapi.LoadLib("/opt/mqm/lib64/compat", l);
    }
    if (err) {
      err = mqnapi.LoadLib("/opt/mqm/lib", l);
    }
    if (err && !searchRedistFirst) {
      err = mqnapi.LoadLib(p, l);
    }
...
function _connx(jsqMgrName, jscno, async, cb) {
  ...
  if (async) {
    log.debug("_connx - jscno: %o",jscno)
...
function put(jsObject, jsmd, jspmo, buf, async, cb) {
  ...
  function putInternal() {
    ...
    var savedMDMsgFlags = jsmd.MsgFlags;
    var savedMDReport = jsmd.Report;
    var savedPMOOptions = jspmo.Options;

    jsmd.MsgFlags = mqt.flagsToNumber('MQMF', jsmd.MsgFlags);
    jsmd.Report = mqt.flagsToNumber('MQRO', jsmd.Report);
    jspmo.Options = mqt.flagsToNumber('MQPMO', jspmo.Options);
    ...
    if (async) {
      mqnapi.Put(jsObject._mqQueueManager._hConn,
        ...
        buf, function (result) {
          ...
          // Reset these fields to the same format (string based or integer) as we had coming in
