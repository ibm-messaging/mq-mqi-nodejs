 ...
 * function.
 *
 * <p>Unimplemented MQI verbs include MQBEGIN, and all of the
 * message property controls.
 *
...
function MQError(mqcc,mqrc,verb)  {
  ...
  if (!mqccstr)
    mqccstr = "Unknown";
  this.message = util.format("%s: MQCC = %s [%d] MQRC = %s [%d] [Pkg ver = %s]", verb,mqccstr,mqcc,mqrcstr,mqrc,packageVersion);
...
exports.Subrq = function(jsQueueManager,jsSubObject,jsaction,jssro,cb) {
  checkParamCB(cb);
  checkParam(jsQueueManager, MQQueueManager, 'MQQueueManager');
  checkParamOptional(jsSubObject, MQObject,'MQObject',true);
 ...
 * <p>Note that GetDone() must not be called directly from within the
 ...
 * late enough.
 * <p>It has to be after the real callback (mqiCB in this module) has returned because of
 * the MQ internal thread controls - MQ thinks it has called us on thread X while, via FFI,
 ...
 * <p>That constraint actually applies to all MQI verbs being called within the callback
 * though for now only the GetDone() [MQCTL/MQCB] is being policed.
 * You similarly cannot call Close() or Disc() from the callback - attempts to do so
 * may result in an MQCC failed return, or more likely the program apparently hanging
 * as the MQ library is blocking on the main thread. All such operations need to be
 * scheduled to run after the callback itself has completed.
 *
 * @param {MQObject}
 *        jsObject - reference to the opened object (hConn and hObj)
 * @param {MQMD}
 ...
 */
process.on('exit',function(){fficb;}); //jshint ignore:line
 ...
 *        attributes to look for (MQIA*, MQCA* values)
 * @param {Array.<number>}
 *        jsIntAttrs - Array that will contain the returned integer values. The
 *        array may be resized and its contents updated during this call.
 * @param {Buffer}
 *        jsCharAttrBuffer - Buffer of sufficient size to hold returned string attribute
 *        values.
 * @param {function}
 *        callback - Called when the inquire completes. Parameters are (error)
 * @throws {MQError}
 *          Container for MQRC and MQCC values
 * @throws {TypeError}
 *          When a parameter is of incorrect type
 */
exports.Inq = function(jsObject,jsSelectors,jsIntAttrs,jsCharAttrBuffer,cb) {
  checkParamCB(cb);
  checkParam(jsObject, MQObject, 'MQObject',true);
  checkParam(jsCharAttrBuffer,Buffer,'Buffer');
  checkArray(jsSelectors);

  var mqRc = ref.alloc(MQT.LONG);
  var mqCc = ref.alloc(MQT.LONG);
  var mqSelectors = new mqLongArray(jsSelectors.length);
  for (var i=0;i<jsSelectors.length;i++) {
    mqSelectors[i] = jsSelectors[i];
  }
  ...
  var ptr = ref.NULL_POINTER;
  if (jsCharAttrBuffer) {
    charAttrLen = jsCharAttrBuffer.length;
    if (charAttrLen > 0) {
      ptr = jsCharAttrBuffer;
    }
  }
  ...
  for (i=0;i<mqIntAttrs.length;i++) {
    jsIntAttrs[i] = mqIntAttrs[i];
  }
 ...
 * @param {Array.<number>}
 *        jsSelectors - Array containing the selectors naming the
 *        attributes to change (MQIA*, MQCA* values)
 * @param {Array.<number>}
 *        jsIntAttrs - Array containing the values for the integer attributes (from
 *        the MQIA selectors).
 * @param {Buffer}
 *        jsCharAttrBuffer - Buffer holding the string attribute values, concatenated
 *        and with space or null paddings to ensure each attribute is the full length.
 * @param {function}
 ...
 *          When a parameter is of incorrect type
 */
exports.Set = function(jsObject,jsSelectors,jsIntAttrs,jsCharAttrBuffer,cb) {
  checkParamCB(cb);
  checkParam(jsObject, MQObject, 'MQObject',true);
  checkParam(jsCharAttrBuffer,Buffer,'Buffer');
  checkArray(jsSelectors);
  checkArray(jsIntAttrs);
  ...
  // Allocate an array to hold the values of the integer attributes
  var mqIntAttrs;
  var mqIntAttrsLen = 0;
  if (jsIntAttrs) {
    mqIntAttrs = new mqLongArray(jsIntAttrs.length);
    mqIntAttrsLen = mqIntAttrs.length;
    for (i=0;i<mqIntAttrs.length;i++) {
      mqIntAttrs[i] = jsIntAttrs[i];
    }
  }
