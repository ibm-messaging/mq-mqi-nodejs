 ...
 *
 * @author Mark Taylor
 * @copyright Copyright (c) IBM Corporation 2017, 2022
 */
...
function loadLib(dir,name) {
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      ...
      debugLog("MQ library '%s' loaded from %s combined with [%s]",name,
                                                                (dir==null)?"<<default path>>":dir,
                                                                (ldPath==null)?"<<empty DYLD path>>":ldPath);
    } else {
      debugLog("MQ library '%s' loaded from %s",name,(dir==null)?"<<default path>>":dir);
    }

    return null;
  } catch(err) {
    debugLog("MQ library '%s' failed to be loaded from %s. Error %s",name,(dir==null)?"<<default path>>":dir,err.message);
    return err;
  }
}
