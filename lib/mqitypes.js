...
class MQError extends Error {
  constructor(mqcc, mqrc, verb) {
    ...
    let mqrcstr = exports.Lookup("MQRC", mqrc);
    if (!mqrcstr)
      mqrcstr = exports.Lookup("MQRCCF", mqrc);
    if (!mqrcstr)
      mqrcstr = "Unknown";
    let mqccstr = exports.Lookup("MQCC", mqcc);
    if (!mqccstr)
      mqccstr = "Unknown";
...
exports.flagsToNumber = function (f,o) {
  ...
  let i;
  let v;
  const err = new TypeError("Invalid type for field " + f);
...
exports.checkArray = function (o) {
  const rc = (!!o) && (o.constructor === Array);
  if (!rc) {
