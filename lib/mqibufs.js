...
exports.bufNewWriteString = function(buf,offsetMap,field,val,len,o) {
  offsetMap[field] = o.offset;
  //console.log("Setting initial value for %s to \"%s\" maxlen=%d offset=%d",field,val,len,offset);
  o.offset +=exports.bufWriteString(buf,val,len,o.offset);
  //console.log("New val = ",buf.subarray(offset,offset+len));
};
...
exports.bufWriteString = function(buf,val,len,offset) {
  var i = 0;
  //console.log("Input string is \"%s\" of type %s actual len = %d req len =%d",val,typeof(val),(val == null)?-1:val.length,len);
  if (val != null) {
...
exports.bufReadString = function(buf,len,offset) {
  //console.log("BufReadString: Full buf is len %d type %s %O ", buf.length,typeof(buf), buf);
  //console.log("BufReadString: extracting from %d for %d returns %O", offset, len,buf.subarray(offset,offset+len));
  var s =  buf.toString('utf8',offset,offset+len);
  s = s.trim();
  //console.log("BufReadString: returning \"%s\"",s);
  return s;
};
