...
function dltHandles(hConn) {
    log.traceEntry("dltHandles");

    for (const [k] of objectHandleMap) {
        const prefix = hConn._hConn + "/";

        if (k.startsWith(prefix)) {
            const mh = objectHandleMap.get(k);
            if (mh) {
                mq.mqi.DltMh(hConn, mh, dmho, function (e) {
                    // Ignore any failure
                });
       			log.debug("Deleting handle key %s", k);
                objectHandleMap.delete(k);
            }
        }
    }

    // And delete information about any OPENed object too
    for (const [k] of objectOptionsMap) {
        const prefix = hConn._hConn + "/";

        if (k.startsWith(prefix)) {
            const mh = objectOptionsMap.get(k);
			if (mh && mh.managedHObj) {
				const managedKey = hConn._hConn + "/" + mh.managedHObj;
                log.debug("Deleting managed options key %s", managedKey);
                objectOptionsMap.delete(managedKey);
			}

   			log.debug("Deleting options key %s", k);
            objectOptionsMap.delete(k);
        }
    }
    log.traceExit("dltHandles");
}
...
function openAfter(hConn, hObj, od, openOptions, managedHObj) {
    log.traceEntry("openAfter");

    log.debug("openAfter managedHObj: %o", managedHObj);
    ...
    if (((od && od.ObjectType == MQC.MQOT_Q) && (openOptions & openGetOptions) != 0) || (managedHObj != null)) {
        ...
        if ((managedHObj == null) && (openOptions & MQC.MQOO_INQUIRE)) {
        ...
        } else if (managedHObj != null) {
            log.debug("Looking at managed hObj");
             try {
                mq.mqi.Inq(managedHObj, selectors);
                log.debug("openAfter: Inq err return: %o", selectors);
                propCtl = selectors[0].value;
            } catch (err) {
                log.debug("openAfter: Inq err %o", err);
            ...
            const options = { propCtl: propCtl, managedHObj: null };
            // replace any existing value for this object handle
            const managedKey = hcKey(hConn, managedHObj);
            log.debug("Setting options map for managed %s", managedKey);
            objectOptionsMap.set(managedKey, options);
        } else {
        ...
        const options = { propCtl: propCtl, managedHObj: managedHObj?managedHObj._hObj:null };
        // replace any existing value for this object handle
        log.debug("Setting options map for %s", key);
...
function closeAfter(hObj) {
    ...
    if (objectOptionsMap.has(key)) {
        const mh = objectOptionsMap.get(key);
        if (mh && mh.managedHObj) {
       	  const managedKey = hObj._mqQueueManager._hConn + "/" + mh.managedHObj;
          log.debug("Deleting managed options key %s", managedKey);
          objectOptionsMap.delete(managedKey);
        }
        log.debug("Deleting options key %s", key);
