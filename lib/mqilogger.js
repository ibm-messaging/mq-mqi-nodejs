...
function debug(fmt,...args) {
  ...
  if (exports.debugEnabled) {
    ...
    const l = util.format("[ibmmq] (D) %s : " + fmt,new Date().toISOString(),...args);
    console.error(l);
  }
}

function error(fmt,...args) {
    const l = util.format("[ibmmq] (E) %s : " + fmt,new Date().toISOString(),...args);
    console.error(l);
}
...
function trace(fn,inout,fmt,...args) {
  ...
  if (exports.traceEnabled) {
    ...
    const l = util.format("[ibmmq] (T) %s : %s " + fmt,new Date().toISOString(),traceFormatFn(inout,fn),...args);
    console.error(l);
  }
}
