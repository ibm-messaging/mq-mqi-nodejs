...
exports.GetAsync = function (jsObject, jsmd, jsgmo, appCB) {
  ...
  log.debug("OtelOpts: %o",jsgmo.OtelOpts);
...
function preJsAppCB(o, buf) {
  ...
  log.debug("OtelOpts returned: %o",otelOpts);
  ...
  if (appCB) {
    ...
    const removed = mqotel.getOtelTraceAfter(hConn, hObj, jsgmo, jsmd, buf?buf.subarray(s):null, true, otelOpts);
    log.debug("Removed: %d",removed);
    appCB(err, hObj, jsgmo,jsmd,buf?buf.subarray(s + removed):null, hConn );
    if (hConn) {
        hConn._inCB=false;
    }
