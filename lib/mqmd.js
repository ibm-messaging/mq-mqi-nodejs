 ...
 * {@link https://www.ibm.com/docs/en/ibm-mq/latest?topic=mqi-mqmd-message-descriptor|MQ Documentation}
...
const mdOffsets = {};
const md_default = Buffer.alloc(MQC.MQMD_LENGTH_2);
...
function createDefaultMDBuf() {
  ...
  if (o.offset != md.length) {
      console.error("Bad length for MQMD - check initial structure fields");
  }
  return md;
}

function _newBuf() {
  const buf = Buffer.alloc(MQC.MQMD_LENGTH_2);
  md_default.copy(buf);
  return buf;
}
...
function _copyMDfromBuf(jsmd, mqmd) {
  ...
  const flags = b.bufReadInt32(mqmd, mdOffsets.MsgFlags);
