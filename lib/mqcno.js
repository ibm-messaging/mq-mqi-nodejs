...
var _MQCNOffi_t = StructType({
  ...
  ApplName          : MQT.CHAR28 ,
  Reserved2         : MQT.BYTE4,
  BalanceParmsPtr   : MQT.PTR,
  BalanceParmsOffset: MQT.LONG,
  Reserved3         : MQT.BYTE4
});
...
exports._copyCNOtoC = function(jscno) {
  ...
  mqcno.Options          = u.flagsToNumber('MQCNO',jscno.Options);

  // Make sure the right threading option is set
  if ((mqcno.Options & (MQC.MQCNO_HANDLE_SHARE_NO_BLOCK |
                       MQC.MQCNO_HANDLE_SHARE_BLOCK)) == 0) {
    mqcno.Options |= MQC.MQCNO_HANDLE_SHARE_BLOCK;
  }
  ...
  if (jscsp != null) {
    ...
    // There's only one non-default option for now. We don't copy it into the C structure
    // if the userid is empty because the CSP might be being used only for
    // the InitialKey setting.
    ...
    if (jscsp.InitialKey != null) {
      mqcsp.InitialKeyPtr = ref.allocCString(jscsp.InitialKey);
      mqcsp.InitialKeyOffset = 0;
      mqcsp.InitialKeyLength = jscsp.InitialKey.length;
      if (mqcsp.Version < 2) {
        mqcsp.Version = 2;
      }
    }
  ...
  if (jsbno != null) {
    ...
    var mqbno = MQBNO._newMQBNOffi();
    mqbno.ApplType = jsbno.ApplType;
    mqbno.Timeout  = jsbno.Timeout;
    mqbno.Options  = u.flagsToNumber('MQBNO_OPTIONS',jsbno.Options);

    mqcno.BalanceParmsPtr = mqbno.ref();
    if (mqcno.Version < 8) {
      mqcno.Version = 8;
    }
  }
...
exports._copyCNOfromC = function(mqcno, jscno) {
  ...
  */
   ...
   /* There are no MQBNO fields that need to be copied back or freed */
