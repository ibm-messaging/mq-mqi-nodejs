...
function getMessage(hObj: mq.MQObject) {
  ...
  try {
    ...
    if (mqerr.mqrc == MQC.MQRC_NO_MSG_AVAILABLE) {
      console.log("no more messages");
    } else {
      console.log(formatErr(mqerr));
    }
  }
}

function printMessage(format: mq.MQFMT, buf: Buffer, len: number) {
  if (format == "MQSTR") {
...
mq.Connx(qMgr, cno, function (connxErr, hConn) {
  if (connxErr) {
    console.log(formatErr(connxErr));
  } else {
